...
function readB64(name) { return readFileSync(join(dir, name), 'utf8').trim(); }
for (const [file, dest] of Object.entries(icons)) {
  const buf = Buffer.from(readB64(file), 'base64');
  if (buf.length !== sizes[dest]) throw new Error(`${dest}: got ${buf.length}`);
