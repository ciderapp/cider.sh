...
<template>
  <main class="overflow-hidden">
    <div class="spin relative mx-auto flex h-max w-max items-center justify-center opacity-50 blur-[120px] filter">
      <div class="absolute inset-0 flex items-center justify-center">
        <div class="absolute h-[40rem] w-[40rem] rounded-full bg-rose-600" style="transform: translate(-20%, 0)"></div>
        <div class="bg-cider absolute h-[40rem] w-[40rem] rounded-full" style="transform: translate(10%, -35%)"></div>
    ...
    <UiContainer class="relative flex flex-col items-center py-10 text-center lg:py-20">
      <div v-if="data">
        <UiBadge :to="data[0]?._path" variant="secondary"
      ...
      <div class="flex flex-col-reverse md:flex-row">
        <div :class="[isUwu ? 'w-full max-w-[760px] max-w-xl justify-between !text-left' : '']">
          <h1 class="mb-4 mt-7 text-4xl font-extrabold lg:mb-6 lg:mt-5 lg:text-center lg:text-5xl xl:text-6xl"
          ...
          <div class="mt-8 flex w-full items-center justify-center gap-3 lg:mt-10"
            :class="[isUwu ? 'md:justify-start md:text-left' : '']">
            <NuxtLink to="/downloads">
              <UiButton size="lg">Get Cider</UiButton>
            </NuxtLink>
            <NuxtLink to="/learn-more">
              <UiButton size="lg" variant="ghost" class="learn-more hover:gap-5 hover:bg-transparent hover:underline">
                Learn More
        ...
        <template v-for="(f, i) in features" :key="i">
          <div class="group flex flex-col items-center justify-center">
            <div class="flex h-12 w-12 items-center justify-center rounded-md border">
              <Icon :name="f.icon" class="h-5 w-5 transition-colors group-hover:text-primary lg:h-6 lg:w-6" />
            </div>
            <h3 class="mt-4 text-balance text-center text-lg font-semibold lg:mt-5 lg:text-xl" v-html="f.title" />
