<template>
  <UiContainer class="relative flex flex-col items-start justify-center gap-10 gap-y-12 py-10 lg:h-[550px] lg:flex-row">
    ...
    <div class="flex h-[350px] w-full shrink-0 items-end p-2 md:w-[700px] lg:h-full lg:p-8">
      <LazySwiper :autoplay="{ delay: 4000 }" :modules="[SwiperAutoplay]" :speed="800" loop grab-cursor
        ...
        <SwiperSlide v-for="t in testimonies" :key="t.author">
          <div
            class="light:border-black/20 light:bg-gray-700/10 rounded-lg border border-white/20 bg-gray-500/10 p-4 text-foreground backdrop-blur-lg lg:px-5 lg:py-6 h-[300px] flex flex-col">
            <div class="mb-5 text-xl grow inline-flex">
              <p class="italic" v-html="addQuotes(t.quote)" />
            </div>
